# whereabouts
Branch from v0.5.4 of whereabouts project. This repo added an incremental IP allocation mode to avoid the case that the application Pod does not recognize the old IP that has been revoked and allocated to another Pod, causing connection errors between applications.
...
### An example configuration using a `NetworkAttachmentDefinition` with sequence mode 

Sequence mode allocates according to the increasing IP trend to avoid the case that the application Pod does not recognize the old IP that has been revoked and allocated to another Pod, causing connection failure between applications.
