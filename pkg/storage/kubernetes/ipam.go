...
func toIPPointerList(pointers map[string]whereaboutsv1alpha1.Pointer) ([]whereaboutstypes.IPPointerReservation) {
	reservedpointerlist := []whereaboutstypes.IPPointerReservation{}
	for i, v := range pointers {
		reservedpointerlist = append(reservedpointerlist, whereaboutstypes.IPPointerReservation{NetworkName: i, IPPointer: net.ParseIP(v.IP)})
		//logging.Debugf("tupt18: appending ip pointer %s: %v", i, v.IPPointer)
	}
	logging.Debugf("tupt18: updated reservedpointerlist: %v", reservedpointerlist)
	return reservedpointerlist
}
...
func toPointerMap(pointerlist []whereaboutstypes.IPPointerReservation) map[string]whereaboutsv1alpha1.Pointer {
	pointers := make(map[string]whereaboutsv1alpha1.Pointer)
	if pointerlist != nil {
		for _, r := range pointerlist {
			index := r.NetworkName 
			pointers[index] = whereaboutsv1alpha1.Pointer{IP: r.IPPointer.String()}
		}
		logging.Debugf("tupt18: pointers after running toPointerMap(): %v", pointers)
	}
	return pointers
}
...
func (p *KubernetesIPPool) GetPointer() []whereaboutstypes.IPPointerReservation {
	return toIPPointerList(p.pool.Spec.Pointers)
}

// Update sets the pool allocated IP list to the given IP reservations
func (p *KubernetesIPPool) Update(ctx context.Context, reservations []whereaboutstypes.IPReservation, pointers []whereaboutstypes.IPPointerReservation) error {
	// marshal the current pool to serve as the base for the patch creation
	orig := p.pool.DeepCopy()
	origBytes, err := json.Marshal(orig)
	//logging.Debugf("tupt18: origBytes content \n %s", origBytes)
	if err != nil {
		return err
	}
	...
	//update the pointers before marshalling once again
	logging.Debugf("tupt18: pointers content \n %v", pointers)
	p.pool.Spec.Pointers = toPointerMap(pointers)
	modBytes, err := json.Marshal(p.pool)
	if err != nil {
		return err
	}
	//logging.Debugf("tupt18: modBytes content \n %s", modBytes)
	// create the patch
	patch, err := jsonpatch.CreatePatch(origBytes, modBytes)
	if err != nil {
		return err
	}
	//logging.Debugf("tupt18: patch content \n %v", patch)
	...
	//logging.Debugf("tupt18: ops content \n %v", ops)

	//value_pt := map[string]string{"ippointer":pt}
	// add additional pointers to the patch
	//ops = append(ops, jsonpatch.Operation{Operation: "add", Path: "/spec/pointers" + networkname, Value: value_pt})

	patchData, err := json.Marshal(ops)
	if err != nil {
		return err
	}
	logging.Debugf("tupt18: patchData content \n %s", patchData)
...
func IPManagementKubernetesUpdate(ctx context.Context, mode int, ipam *KubernetesIPAM, ipamConf whereaboutstypes.IPAMConfig,
	...
	var newip net.IPNet
	var updatedpointerlist []whereaboutstypes.IPPointerReservation
	// Skip invalid modes
	switch mode {
...
RETRYLOOP:
	for j := 0; j < storage.DatastoreRetries; j++ {
		...
		logging.Debugf("tupt18: pool %s", pool)
		if err != nil {
		...
		pointerlist := pool.GetPointer()
		
		logging.Debugf("tupt18: pointerlist %v", pointerlist)
		logging.Debugf("tupt18: reservelist %v", reservelist)

		var updatedreservelist []whereaboutstypes.IPReservation
		switch mode {
		case whereaboutstypes.Allocate:
			newip, updatedreservelist, updatedpointerlist, err = allocate.AssignIP(ipamConf, reservelist, containerID, podRef, pointerlist)
			logging.Debugf("tupt18: ipamConf.Name is %s", ipamConf.Name)
			logging.Debugf("tupt18: updatedpointerlist after running assignIP, pt = %s", updatedpointerlist)
			logging.Debugf("tupt18: updatedreservelist after running assignIP, pt = %s", updatedreservelist)
			if err != nil {
		...
		case whereaboutstypes.Deallocate:
			...
			// if deallocate happen, pointers is unchange
			updatedpointerlist = pointerlist
			if err != nil {
		...
		err = pool.Update(requestCtx, usereservelist, updatedpointerlist)
		if err != nil {
