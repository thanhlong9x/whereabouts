...
func (i *ETCDIPAM) GetIPPool(ctx context.Context, ipRange string) (IPPool, error) {
	reservelist, pointerlist, err := i.getRange(ctx, ipRange)
	if err != nil {
		return nil, err
	}
	return &ETCDIPPool{i.kv, ipRange, reservelist, pointerlist}, nil
}

// GetRange gets the reserved list of IPs for a range
func (i *ETCDIPAM) getRange(ctx context.Context, iprange string) ([]types.IPReservation, []types.IPPointerReservation, error) {
	...
	list := make([]types.IPReservation, count)
	list_pointer := make([]types.IPPointerReservation, len(reservations)- count)
	...
	for i, raw := range reservations {
		...
		if i < count {
			...
			list_pointer[i-count] = types.IPPointerReservation{
				NetworkName: string(split[0]),
				IPPointer: net.ParseIP(split[1]),
			}
		}
	}

	return list, list_pointer, nil
}
...
type ETCDIPPool struct {
	...
	pointers    []types.IPPointerReservation
}

func (p *ETCDIPPool) GetPointer() []types.IPPointerReservation {
	return p.pointers
}
...
func (p *ETCDIPPool) Update(ctx context.Context, reservations []types.IPReservation, pointers []types.IPPointerReservation) error {
	var raw []string
	for _, r := range reservations {
		raw = append(raw, fmt.Sprintf("%s %s", r.IP.String(), r.ContainerID))
	}
	for _, r := range pointers {
		raw = append(raw, fmt.Sprintf("%s %s", r.NetworkName, r.IPPointer.String()))
	}
	_, err := p.kv.Put(ctx, fmt.Sprintf("%s/%s", whereaboutsPrefix, p.ipRange), strings.Join(raw, "\n"))
	// if err != nil {
	// 	return err
	// }
	// _, err := p.kv.Put(ctx, fmt.Sprintf("%s/%s/Pointers", whereaboutsPrefix, p.ipRange), strings.Join(raw, "\n"))
	return err
}
...
RETRYLOOP:
	for j := 0; j < DatastoreRetries; j++ {
		...
		var updatedpointerlist []types.IPPointerReservation
		...
		err = pool.Update(requestCtx, updatedreservelist, updatedpointerlist)
		if err != nil {
