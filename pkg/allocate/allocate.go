...
func AssignIP(ipamConf types.IPAMConfig, reservelist []types.IPReservation, containerID string, podRef string, pointer []types.IPPointerReservation) (net.IPNet, []types.IPReservation, []types.IPPointerReservation, error) {

	// Setup the basics here.
	_, ipnet, _ := net.ParseCIDR(ipamConf.Range)

	newip, updatedreservelist, updatedpointerlist, err := IterateForAssignment(*ipnet, ipamConf.RangeStart, ipamConf.RangeEnd, reservelist, ipamConf.OmitRanges, containerID, podRef, pointer, ipamConf.Name, ipamConf.Mode)
	if err != nil {
		return net.IPNet{}, nil, updatedpointerlist, err
	}
	logging.Debugf("tupt18: updatedreservelist at assignIP func after assigning: %s", updatedreservelist)
	logging.Debugf("tupt18: updatedpointerlist at assignIP func after assigning: %s", updatedpointerlist)
	return net.IPNet{IP: newip, Mask: ipnet.Mask}, updatedreservelist, updatedpointerlist, nil
}
...
func IterateForAssignment(ipnet net.IPNet, rangeStart net.IP, rangeEnd net.IP, reservelist []types.IPReservation, excludeRanges []string, containerID string, podRef string, pointer []types.IPPointerReservation, networkname string, mode string) (net.IP, []types.IPReservation, []types.IPPointerReservation, error) {
	firstip := rangeStart.To16()
	var lastip net.IP
	logging.Debugf("tupt18: mode: %s", mode)
	if rangeEnd != nil {
		...
		if err != nil {
			...
			return net.IP{}, reservelist, pointer, err
		}
	}
	startpoint := firstip
	logging.Debugf("IterateForAssignment input >> ip: %v | ipnet: %v | first IP: %v | last IP: %v", rangeStart, ipnet, firstip, lastip)
	logging.Debugf("tupt18: pointerlist argument: %v", pointer)

	isExistedNetwork := false
	if mode == "sequence" {
		logging.Debugf("tupt18: mode == pointer active: %v", pointer)
		for _, v := range pointer {
			if v.NetworkName == networkname {
				isExistedNetwork = true
				if v.IPPointer != nil {
					if bytes.Compare(v.IPPointer, firstip) >= 0 && bytes.Compare(v.IPPointer, lastip) <= 0 {
						startpoint = v.IPPointer
						break
					}
				}
			}
		}
		logging.Debugf("tupt18: startpoint in pointer mode: %v", startpoint)
	}
	...
	for i := startpoint; !i.Equal(endip); i = IPAddOffset(i, uint64(1)) {
		...
		assignedip = i
		if IPAddOffset(i, uint64(1)).Equal(endip) {
			startpoint = firstip
		} else {
			startpoint = IPAddOffset(i, uint64(1))
		}
	...
	if !performedassignment {
		for i := firstip; !i.Equal(startpoint); i = IPAddOffset(i, uint64(1)) {
			// if already reserved, skip it
			if reserved[i.String()] {
				continue
			}
	...
	logging.Debugf("tupt18: reservelist after assign %s", reservelist)

	if !performedassignment {
		return net.IP{}, reservelist, pointer, AssignmentError{firstip, lastip, ipnet}
	}

	//update new ip for pointer of network
	var kq []types.IPPointerReservation

	for _, v := range pointer {
		if v.NetworkName == networkname {
			if mode == "sequence" {
				kq = append(kq, types.IPPointerReservation{NetworkName: networkname, IPPointer: startpoint})
			}
		} else {
			kq = append(kq, types.IPPointerReservation{NetworkName: v.NetworkName, IPPointer: v.IPPointer})
		}
	}
	if !isExistedNetwork && mode == "sequence" {
		kq = append(kq, types.IPPointerReservation{NetworkName: networkname, IPPointer: startpoint})
	}

	logging.Debugf("tupt18: updated startpoint after assigning %s", startpoint)
	//logging.Debugf("tupt18: pointer after assigning %v", kq)
	return assignedip, reservelist, kq, nil
}
