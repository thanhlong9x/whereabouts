...
func (dp dummyPool) Update(context.Context, []types.IPReservation, []types.IPPointerReservation) error {
	return nil
}

func (dp dummyPool) GetPointer() []types.IPPointerReservation {
	return dp.pointers
}
...
var _ = Describe("IPReconciler", func() {
	...
	When("there are IP addresses to reconcile", func() {
		...
		BeforeEach(func() {
			...
			ippointer := generateIPPointerReservation("test-network", firstIPInRange)
			pool := generateIPPool(ipCIDR, podRef)
			orphanedIPAddr := OrphanedIPReservations{
				Pool:        dummyPool{orphans: reservations, pool: pool, pointers: ippointer},
...
func generateIPPointerReservation(name string, ip string) []types.IPPointerReservation {
	return []types.IPPointerReservation{
		{
			NetworkName: name,
			IPPointer: 	 net.ParseIP(ip),
		},
	}
}